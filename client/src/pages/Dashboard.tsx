...
import { BarChart3, Camera, CheckCircle2, CircleDollarSign, Download, Gift, Loader2, LogOut, MonitorUp, ScanLine, Shuffle, Ticket, Trophy, Users, XCircle } from "lucide-react";
...
// Durasi putaran undian (ms) — dipakai bareng oleh dashboard & layar undian.
const ROLL_MS = 4200;
...
export default function Dashboard() {
  ...
  const guestsQuery = trpc.event.guests.useQuery(undefined, { enabled: Boolean(user) });
  ...
  const redeemMutation = trpc.event.redeem.useMutation({ onSuccess: result => { lastScanRef.current = { ...lastScanRef.current, at: Date.now() }; scanLockedRef.current = false; setScannedCode(""); setScanNotice(`Redeem berhasil untuk ${result.guest.name}.`); void query.refetch(); }, onError: () => { scanLockedRef.current = false; } });
  const data = query.data;
  const selectedPrize = data?.prizes.find(prize => prize.id === selectedPrizeId);
  const filteredGuests = (guestsQuery.data ?? []).filter(guest => {
    const q = guestSearch.trim().toLowerCase();
    if (!q) return true;
    return (
      guest.name.toLowerCase().includes(q) ||
      guest.registrationCode.toLowerCase().includes(q) ||
      guest.department.toLowerCase().includes(q)
    );
  });
  ...
  const handleScanResult = (rawCode: string) => {
    ...
    playScanFeedback();
    // Langsung tandai hadiah sebagai diambil (tanpa langkah validasi terpisah).
    redeemMutation.mutate({ registrationCode: code });
  ...
  const openDrawDisplay = () => {
    const win = window.open("/layar-undian", "sasa-doorprize-display", "popup,width=1440,height=900");
    if (win) displayWindowRef.current = win;
    return win;
  };

  const ensureDisplayWindow = () => {
    if (displayWindowRef.current && !displayWindowRef.current.closed) return displayWindowRef.current;
    return openDrawDisplay();
  };
  ...
  const runDraw = async () => {
    ...
    // Kalau panitia belum pilih hadiah, pakai hadiah pertama yang kuotanya masih ada.
    let prizeId = selectedPrizeId;
    let prize = selectedPrize;
    if (!prizeId || !prize) {
      const fallback = (data?.prizes ?? []).find(item => item.drawn < item.quantity);
      if (!fallback) return;
      prizeId = fallback.id;
      prize = fallback;
      setSelectedPrizeId(fallback.id);
    }
    // Buka layar undian otomatis (kalau belum kebuka) supaya roller kelihatan.
    ensureDisplayWindow();
    setWinner(null);
    setIsShuffling(true);
    ...
    try {
      ...
      // Putaran melambat: cepat di awal, makin lambat menjelang akhir (ease-out).
      const startAt = performance.now();
      const scheduleNext = () => {
        const t = Math.min(1, (performance.now() - startAt) / ROLL_MS);
        const delay = 55 + Math.pow(t, 2.6) * 650;
      ...
      scheduleNext();
      await new Promise(resolve => window.setTimeout(resolve, ROLL_MS));
  ...
  const exportXls = () => {
    ...
    const rows = list.map((guest, i) => ({
      No: i + 1,
      "Kode Registrasi": guest.registrationCode,
      Nama: guest.name,
      "Divisi/Departemen": guest.department,
      "No Handphone": guest.phone,
      Pemenang: guest.isWinner ? "Ya" : "Tidak",
      Hadiah: guest.prizeName ?? "-",
      "Waktu Menang": fmt(guest.winnerAt),
      "Sudah Diambil": guest.redeemedAt ? "Ya" : "Tidak",
      "Waktu Diambil": fmt(guest.redeemedAt),
      "Diambil Oleh": guest.redeemedBy ?? "-",
      "Terdaftar": fmt(guest.createdAt),
    }));
    const worksheet = XLSX.utils.json_to_sheet(rows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Peserta");
    XLSX.writeFile(workbook, `peserta-sasa-${new Date().toISOString().slice(0, 10)}.xls`, { bookType: "xls" });
  };
  ...
  return <div className="app-shell min-h-screen bg-[#f7f8fa] text-[#273049]">
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#e21b22]">Panel panitia / realtime</p><h1 className="brand-display mt-3 text-4xl font-black tracking-[-.045em] sm:text-5xl">Kelola pengambilan hadiah.</h1><p className="mt-3 text-sm leading-6 text-[#6b7280]">Pindai tiket peserta, validasi pemenang, lalu tandai hadiah sebagai sudah diambil.</p></div><div className="flex items-center gap-2 text-xs font-bold text-[#6b7280]"><span className="status-dot" /> Data tersinkron saat halaman dibuka</div></div>
        ...
        <div className="rounded-[28px] bg-[#273049] p-6 text-white shadow-[0_18px_50px_rgba(39,48,73,.16)] sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#ffd949]">Scan tiket</p><h2 className="mt-2 text-2xl font-black">Pindai QR peserta</h2><p className="mt-3 max-w-lg text-sm leading-6 text-white/65">Arahkan kamera ke QR code tiket peserta. Begitu terbaca, hadiah langsung ditandai sebagai sudah diambil — tanpa langkah validasi terpisah.</p></div><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-[#ffd949]"><ScanLine className="h-5 w-5" /></div></div><div className="mt-6 flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><div><p className="text-xs font-black text-white">Mode antrean</p><p className="mt-1 text-[11px] text-white/55">Kamera aktif, lanjut otomatis ke peserta berikutnya</p></div><button type="button" role="switch" aria-checked={continuousMode} onClick={toggleContinuousMode} className={`relative h-7 w-12 rounded-full transition ${continuousMode ? "bg-[#ffd949]" : "bg-white/20"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${continuousMode ? "left-6" : "left-1"}`} /></button></div><div className={`relative mt-6 overflow-hidden rounded-2xl border border-white/10 bg-black/30 ${scannerOpen ? "block" : "hidden"}`}><video ref={videoRef} className="aspect-video w-full object-cover" muted playsInline /><div className="pointer-events-none absolute inset-[18%] rounded-2xl border-2 border-[#ffd949] shadow-[0_0_0_999px_rgba(0,0,0,.22)]" /></div>{!scannerOpen && <div className="mt-6 grid min-h-32 place-items-center rounded-2xl border border-dashed border-white/15 bg-white/5 text-center"><Camera className="h-7 w-7 text-[#ffd949]" /><p className="mt-2 text-xs font-bold text-white/55">Kamera belum aktif</p></div>}{scannerError && <p className="mt-4 rounded-xl bg-[#fff0f0] px-4 py-3 text-xs font-semibold text-[#ffb5b8]">{scannerError}</p>}{redeemMutation.data && <div className="mt-4 rounded-2xl border border-[#b6df9f]/40 bg-[#effae8] px-4 py-3"><p className="text-[10px] font-black uppercase tracking-[.15em] text-[#4d9629]">Hadiah ditandai diambil</p><p className="mt-1 text-sm font-black text-[#273049]">{redeemMutation.data.guest.name} · {redeemMutation.data.guest.registrationCode}</p><p className="mt-0.5 text-xs font-semibold text-[#4d9629]">{redeemMutation.data.prize?.name}</p></div>}{redeemMutation.error && <p className="mt-4 rounded-xl bg-[#fff0f0] px-4 py-3 text-xs font-semibold text-[#ffb5b8]">{redeemMutation.error.message}</p>}{scanNotice && !redeemMutation.data && !redeemMutation.error && <p className="mt-4 rounded-xl bg-white/5 px-4 py-3 text-xs font-semibold text-white/70">{scanNotice}</p>}<button type="button" onClick={scannerOpen ? stopScanner : startScanner} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#ffd949] px-5 py-4 text-sm font-black text-[#273049] transition hover:bg-[#f5c400]">{scannerOpen ? <><XCircle className="h-4 w-4" /> Matikan kamera</> : <><Camera className="h-4 w-4" /> Buka kamera scanner</>}</button></div>
      </section>
